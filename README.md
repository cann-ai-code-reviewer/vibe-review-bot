...
1、推广到2012中央硬件工程院-计算网络技术部（HCCL和HCOMM仓）的分析：因为hccl代码比别的算子组规模更大且存在大量host侧的纯cpp代码（framework、platform），能够产生的检视意见数量更多，理论效果比扫描acl代码更好。个人希望马上就能在hccl小范围（先定50人，我自费给hccl扫代码）部署，需要一些软件&业务高手帮忙分析检视报告，定期（每周）反馈误报+高价值案例+历史dts的分析结果到我这，我统一优化skill。同步地，我最近(三月中)帮中硬自己跑起来，后继续保持合作。
...
2026.2.28：和创新算子部CMC张峰、FA组郭世杰交流了，后续世杰一起参与项目开发。
2026.3.2：和推理组韩秋涵交流了，后续考虑skill拓展到支持python语言&自动化脚本推广到omni仓的话这部分可以由她来vibe。

#### D 运营专题

目标是建立长效机制不断把对误报/高价值缺陷模式的分析结果先反馈到skill再前馈到检视结果，侧重运作机制而非对模式的分析方法。

遗留问题：目前很多人是黄区PC+黄蓝空间开发，在黄区建共享表格的话不方便闭环到蓝区，在蓝区建共享表格的话用啥平台？

- [ ] 采纳率算法优化

#### E 成本专题

目标是降低时间和tok成本，当前效果虽好但成本实在过高（四舍五入平均一个PR十块钱）。

早期阶段全力提升质量，先不管成本，不过有兴趣的同学也可以先思考看看。

2026.3.2：单个PR(+16,-1)直接把世杰的GLM4.7的tok耗完了(约100M)，看来降成本必须提上日程了！
